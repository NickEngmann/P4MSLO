...
- **Main menu layout** — CAMERA, ALBUM, INTERVAL CAM, VIDEO MODE, USB DISK, SETTINGS (in that order). "ALBUM" routes to UI_PAGE_GIFS — it IS the PIMSLO gallery (GIFs + p4ms previews + delete modal). The legacy P4-photo album screen (UI_PAGE_ALBUM) was removed from the menu but the enum + code paths remain compiled so encoder/decoder plumbing still works. AI_DETECT is also hidden from the menu.
