...
| `11_heartbeat.py`         | Page nav over all 4 menu pages, buttons, one spi_pimslo capture, gallery entry + play, sd_ls, heap health, reset_state |
...
# Quick-format the SD card to FAT (true mkfs, ~8 s on 32 GB).
# Refuses if SD missing or any encode/save/capture is in flight.
# Wipes EVERYTHING — PIMSLO captures + p4 album + user files.
format_sd
# → ok format_sd  |  error: format_sd SD not mounted  |  error: format_sd busy (...)
```
...
- **Settings → Format SD button + serial `format_sd` — true FAT mkfs, animated, bg-coordinated, success popup** — Slot 1 of the Settings page (the old OD/Language slot — AI was stripped) is repurposed as a "Format SD" action item. Item label centered, no body text; black/clickable when SD mounted, grey/no-op when missing. SELECT opens a "Format?" YES/NO confirm modal (NO is the safe default). Confirming YES kicks off `format_bg_task` (8 KB BSS-internal stack, Core 1, prio 4 — 4 KB blew up on `app_gifs_scan()` post-format), which: (1) takes the LVGL lock briefly to call `app_gifs_stop()` + `app_gifs_flush_cache()`, (2) releases the lock, (3) calls `app_gifs_signal_bg_abort()` (public helper, sets `s_bg_abort_current` + bumps `s_last_nav_ms` so gif_bg pauses for 15 s), (4) polls `format_workers_idle()` for up to 5 s — `app_gifs_is_encoding() || app_pimslo_is_encoding() || app_pimslo_is_capturing() || app_pimslo_get_queue_depth()>0`, refuses with ESP_ERR_TIMEOUT if anything's still busy, (5) calls `esp_vfs_fat_sdcard_format(BSP_SD_MOUNT_POINT, card)` — true FatFs `f_mkfs` (FM_ANY, FatFs auto-picks variant: <4 GB FAT16, 4-32 GB FAT32, >32 GB exFAT if compiled in), (6) mkdirs the four PIMSLO dirs, (7) takes the lock again briefly for `app_gifs_scan()` + `app_gifs_refresh_empty_overlay()`, (8) `lv_async_call(format_done_async_cb)` to tear down the modal on the LVGL task. The modal swaps title to "Formatting" + animated dots ("Formatting" → "Formatting." → "Formatting.." → "Formatting...") via a 350 ms `lv_timer`. Animation begins immediately when SELECT YES fires (timer created BEFORE the BG task spawns) and continues throughout the bg-abort wait + the 8.5 s `f_mkfs` since the BG task only holds the LVGL lock for ms-scale pre/post bookkeeping. **Success popup**: when format completes, the modal title swaps once more to "Done!" / "Busy" / "No SD" / "Failed" depending on the err code, stays for 1.5 s via a one-shot `lv_timer`, then `format_dismiss_cb` restores the modal to its idle "Format?" + YES/NO state and hides it. While `s_format_in_progress` is true (whole flow including the success popup) the settings page ignores ALL button input (early-bail at top of UI_PAGE_SETTINGS handlers in btn_up/down/menu/encoder) so the user can't navigate away or re-confirm mid-format. **Hardware-verified bg coordination 2026-04-25**: spi_pimslo capture (3/4 cameras → save+encode in flight), waited 5 s into the encode, fired `format_sd` → returned `error: format_sd busy (encoding)`. Encode continued uninterrupted (frame timings 7514 ms / 7717 ms after the refusal); no corruption, no panics. **UI feedback when busy at modal-open**: pressing SELECT on the Format SD slot while bg work is in flight previously was a silent no-op — now `show_format_status_popup("SD Busy", 1500)` reuses the format modal chrome to show a brief 1.5 s "SD Busy" popup so the user gets visible acknowledgment. Important caveat: `pimslo_encode_queue_task` (the first-class photo path) **intentionally ignores `s_bg_abort_current`** (see `app_gifs.c:2240-2256` — gating by `is_bg_caller = xTaskGetCurrentTaskHandle() == s_bg_worker`). That guard exists because test 02 had a race where gallery nav during encode aborted it and produced broken .gifs. Net effect: a freshly-fired photo's encode (~150 s) cannot be interrupted by format — the user must wait. If interrupting becomes important, add a separate "format-priority abort" flag the encode honors regardless of caller. Other caveats: format wipes EVERYTHING on the card including `/sdcard/esp32_p4_pic_save` and any user files; FAT variant auto-picked by FatFs based on volume size (no force-FAT32 unless `CONFIG_FATFS_USE_EXFAT=n` is set in sdkconfig); `pimslo_save_task` may log fwrite errors and advance the queue if format catches it mid-write (tolerated, no crash).
...
- **Main menu layout** — CAMERA, ALBUM, USB DISK, SETTINGS (in that order). "ALBUM" routes to UI_PAGE_GIFS — it IS the PIMSLO gallery (GIFs + p4ms previews + delete modal). VIDEO MODE + INTERVAL CAM were removed from the menu in 2026-04-26 (out of product scope) but the enum values, page-mapping table, and underlying code paths remain compiled, so `menu_goto video_mode` / `menu_goto interval` still work for diagnostics. The legacy P4-photo album screen (UI_PAGE_ALBUM) was likewise removed from the menu earlier but its enum + code paths remain.
...
- **First-photo-after-boot heap corruption — stale cached buffer pointers** — RESOLVED (2026-04-26). `app_video_photo.c` and `app_video_record.c` cached `scaled_camera_buf` / `jpg_buf` / `photo_buf` at file scope from `app_video_photo_init()`. Every PIMSLO photo cycle calls `app_video_stream_free_buffers()` (from `pimslo_capture_task` and from `gif_bg` before its 7 MB encoder alloc) and then `realloc_buffers()` at NEW PSRAM addresses. The cached pointers in the photo/record paths were dangling — pointing at freed PSRAM that had been reissued to the encoder/decoder/canvas cache. Symptom matched the user's "first photo crashes, second works" report exactly: if a bg encode ran between boot and first photo, the cached pointer was already stale; on the next `take_and_save_photo` the JPEG encoder wrote RGB565 source pixels through the stale `pic_buf` → corrupted a TLSF free-block header → next free-walk panic'd (`Store access fault` in `tlsf::remove_free_block`, MTVAL ≈ garbage). Fix: refresh via `app_video_stream_get_*_buf()` at the top of `take_and_save_photo` and `take_and_save_video`. Mock-side: 5 new e2e scenarios in `test/host_encode/test_e2e.c` model the bug (`viewfinder_free_realloc_invalidates_cached_pointer` is the fail-loud BAD path, `viewfinder_consumer_refresh_fixes_stale_pointer` is the fix). Diagnostic that didn't help here: `HEAP_POISONING_LIGHT`'s `0xbaad5678` tail canary catches the corruption AT THE FREE, not at the write — the writer is in a different task / call chain by then. Mock-first iteration was the unlock — sub-second cycles to find the pattern, then port the one-line fix.
- **0-cam-capture ERROR overlay** — when a `photo_btn` returns 0/4 usable JPEGs (cameras off, SPI flake, etc.) the saving overlay used to vanish silently and the user thought the camera was broken. Now `app_pimslo_capture_error_pending()` (new public API in `app_pimslo.h`) returns true for 3 s after a 0-cam capture, `app_pimslo_is_capturing()` ORs that in to keep the saving timer alive, and `saving_timer_cb` in `ui_extra.c` swaps the pill to a red `#c02020` ERROR label for the duration. Capture pipeline clears `s_error_until_ms` on the next successful capture. Mock model in `pimslo_sim.c`: `capture_error_pending` + `simulate_advance_time_ms`. Defines: `PIMSLO_ERROR_OVERLAY_MS = 3000` in `app_pimslo.c`.
- **`gif_quantize.c::CUBE_IDX` 5-bit-spaced macro causing 75 % color-cube collapse** — RESOLVED (2026-04-26). `CUBE_IDX(r6,g6,b6)` was `(r6<<10) | (g6<<5) | b6`, which leaves only 5 bits between fields — the top bit of each 6-bit channel collided with the bottom bit of the next, mapping 196608 of 262144 (r,g,b) tuples to a non-unique bin. The histogram-based palette quantizer was therefore selecting from a 25 %-of-real bin space, causing visibly-wrong colors on certain GIFs. Fix: 6-bit spacing `(r6<<12) | (g6<<6) | b6` (now uses 18 bits, well within int range). Verified by new `test/host_encode/test_quantize_bins.c` — broken formula produces 196608 collisions, fixed produces 0.
- **`tjpgd` workspace + `decode_jpeg_crop` mutex 5 s timeout was too short under contention** — RESOLVED (2026-04-26). Both mutexes used `pdMS_TO_TICKS(5000)`. Under capture+encode+SD concurrent load the 5 s would expire mid-encode, the encoder caller silently treated `ESP_ERR_TIMEOUT` as "skip this frame" and produced a 0-frame `.gif` written to SD. Bumped both to 60 s — better to wait than to ship a broken artifact. Caller never blocks the user-facing UI on these mutexes, so the longer wait is fine. Search `app_gifs_acquire_tjpgd_work` and `decode_jpeg_crop` in `app_gifs.c` / `gif_encoder.c` for the call sites.
- **Gallery-scan orphan cleanup** — `gallery_scan` now drops 0-byte (or <32-byte) `.gif` files outright, and drops PIMSLO capture dirs with <2 pos*.jpg files when no matching `.gif` already exists. Without these checks, interrupted captures left "PROCESSING forever" entries in the gallery (no encoder will pick them up — the encoder requires ≥2 pos files) and 0-byte gifs from prior 5 s mutex timeouts surfaced as broken playback. The cleanup is silent — a `W app_gifs: Cleaning orphan ...` log line is the only signal. Bg encoder failure blacklist (already documented above) handles the runtime case; gallery_scan handles the boot-time / post-format case.
- **Source JPEG cache between Pass 1 and Pass 2 — saves ~7% encode time** — RESOLVED (2026-04-26). Investigation found the encoder was reading each pos*.jpg from SD THREE times: (1) initial load into `jpeg_data[]` near the top of `app_gifs_encode_pimslo_from_dir`, (2) Pass 1 fopen+fread per camera, (3) Pass 2 fopen+fread per camera. The `jpeg_data[]` buffers were freed at line 2489 right before Pass 1 with the comment "they'll be re-read from SD for each pass" — that was wasted I/O. SD reads at ~250 KB/s × 600 KB = ~2.4 s per JPEG, so 4 cams × 2 redundant passes = ~19 s waste per encode. Fix: keep `jpeg_data[]` alive through Pass 1 + Pass 2, free in cleanup. The buffers are 4 × ~600 KB scattered allocations (NOT contiguous with each other) so they don't compete with the encoder's 7 MB scaled_buf alloc. Mock-validated: total encode time 279.1 s → 259.9 s = **6.9% saved (~19 s)**, single-core, always-on benefit. Stacks with the dual-core sleep boost (additional ~3.6%) for **~10% combined when sleeping**. Implementation in `app_gifs.c::app_gifs_encode_pimslo_from_dir`: removed the early free; Pass 1 + Pass 2 now use `jpeg_data[i]` / `jpeg_size[i]` directly via the existing `..._add_frame_from_buffer` APIs. Hardware-verified — full regression PASS, encode log shows no fopen on pos*.jpg during Pass 1/2.
- **Encoder dual-core boost — mock validated, hardware deferred** — INVESTIGATED (2026-04-26). User asked "when we turn off the LCD, can we leverage the now-idle Core 0 (LVGL paused) for faster encoding?" Mock framework added in `p4_timing.c` + `pimslo_sim.c` with a `boost_dual_core` flag sourced from `power_idle_is_sleeping()`. Modeled three concrete parallelization opportunities: (1) `.p4ms` save splits 4 cams 2/2 across cores (40% saved on that phase); (2) Pass 1 palette accumulation overlaps with decode (8% saved); (3) Pass 2 forward SD prefetches the next JPEG on Core 0 while Core 1 dithers (3% saved). 4 new e2e scenarios in `test_e2e.c` cover boost-active-only-when-sleeping, sleep-encode-faster-than-awake (with a >=1% headline assertion), boost-doesn't-break-correctness, and wake-mid-drain-disables-boost. **Realistic hardware win: ~3.6% (~10 s on a ~260 s encode).** The reason it's small: Pass 2 forward dominates (227 s of the 260 s total), and we cannot true-parallel Pass 2 because the 7 MB `scaled_buf` cannot be double-buffered (would need 14 MB contig PSRAM; max contig is ~8 MB). Bigger wins would require a streaming-encoder rewrite that doesn't materialize a full-frame intermediate — major refactor, out of scope. Decision: SHIP THE MOCK FRAMEWORK (boost plumbing, telemetry, scenarios), DEFER FIRMWARE. The boost flag, encoder counters, and scenario harness are in place if/when a future encoder rewrite makes deeper parallelism realizable. The current encoder runs single-threaded on Core 1 regardless of sleep state — same behavior as before this investigation.
- **S3 cameras sleep on `SPI_CMD_SLEEP` (0x0A), wake on trigger pulse** — ADDED (2026-04-26). The 3-min idle power-saving feature now extends to the 4 S3 cameras: when the P4 enters POWER_SLEEPING, it broadcasts `SPI_CAM_CMD_SLEEP` to all 4 S3s. Each S3's handler arms `gpio_wakeup_enable(TRIGGER_PIN=GPIO1, GPIO_INTR_LOW_LEVEL)` + `esp_sleep_enable_gpio_wakeup()` and calls `esp_light_sleep_start()`. Power drops from ~50 mA per S3 to ~0.8 mA — over 4 cams that's ~200 mA saved while the user is idle. Wake mechanism: `ui_extra_redirect_to_camera_page` calls `spi_camera_send_trigger()` immediately after the "Camera mode" overlay shows. The 100 ms LOW pulse on the shared P4 GPIO34 → S3 GPIO1 line wakes all 4 S3s simultaneously via the falling-edge wake source. The 5 s overlay window covers the cams' wake + sensor warm-up time. **Validated** on USB-flashed cam 1: status went 0x00 (sleeping) → 0x01 (woken + JPEG captured) after `trigger 100`. **Caveat — light sleep drops S3 USB CDC**: after sleep + wake, the S3's USB serial endpoint stays disconnected (USB requires CPU activity). SPI continues to work normally for capture / control. This only affects USB serial debugging on the bench-test cam; deployed cams use WiFi-only, no impact. **Caveat — sticky bus state from aggressive testing**: rapid back-to-back sleep/wake cycles can leave the shared GPIO34 trigger line stuck LOW (one cam's GPIO1 holding the bus, multi-cam wake state confused). Per CLAUDE.md "SPI slave stuck in DATA mode" — physical RESET of each affected cam is the only recovery; `cam_reboot` over SPI cannot reach a cam that's holding the bus. The S3-side serial command `cam_sleep [N|all]` exposes the SPI cmd from the P4 for isolated testing without going through the full UI idle flow.
- **3-minute idle → display sleep + LVGL pause** — ADDED (2026-04-26). User-requested power-saving feature: after 3 minutes of no user input on any page, the firmware briefly shows an "Idle / Going to sleep" modal (~1.5 s) on `lv_layer_top`, then turns off the LCD backlight (`bsp_display_backlight_off`) AND stops the LVGL refresh task (`lvgl_port_stop`). The next button press wakes — `lvgl_port_resume` + backlight on + idle timer reset, the wake press is then SWALLOWED (no scroll/select propagation). Background encoding (PIMSLO save + GIF + .p4ms) keeps running on Core 1 throughout sleep — that's the whole power-saving win, freeing display + LVGL CPU for the encoder. State machine: `POWER_ACTIVE → POWER_SLEEP_MODAL → POWER_SLEEPING`. All five button handlers (`btn_up/down/left/right/menu/encoder`) call `wake_from_idle_if_sleeping()` first. Serial-cmd dispatch calls `ui_extra_kick_idle_timer()` so e2e tests don't false-trigger sleep. Sleep is suppressed during active SPI capture (`app_pimslo_is_capturing()`) and during the SD-format BG task (`s_format_in_progress`) — both are short windows where the user is actively waiting on a UI animation. Mock-validated by 7 scenarios in `test/host_encode/test_e2e.c` covering boot-active, timeout-to-modal-to-sleeping, button-resets-timer, wake-during-modal, wake-during-sleeping, encoder-runs-uninterrupted, and reset-clears-state. Constants in `ui_extra.c`: `IDLE_TIMEOUT_MS = 180000`, `IDLE_MODAL_VISIBLE_MS = 1500`, `IDLE_CHECK_INTERVAL_MS = 5000`. Mock has matching `POWER_IDLE_TIMEOUT_MS` and `POWER_SLEEP_MODAL_MS` in `pimslo_sim.h`.
- **JPEG-only gallery entries painted as a blue square** — RESOLVED (2026-04-26). User report: "Entries with just a JPEG that are either in the processing or queued mode aren't showing the still 240×240 JPEG preview. It's just a blue square and the word 'processing/queued' in the center of the screen with the file name on the bottom." Root cause: `factory_demo/main/app/Gif/gif_tjpgd.h` had `#define JD_USE_SCALE 0` from the very first commit (0eefc0a), but `app_gifs.c::show_jpeg()` computes a tjpgd scale (0/1/2/3 for 1:1 / 1:2 / 1:4 / 1:8) based on canvas (240×240) vs source (typically 1920×1080) and lands at scale 2 or 3. tjpgd's `gif_jd_decomp()` returns `JDR_PAR (5)` for any `scale > 0` when `JD_USE_SCALE = 0` (gif_tjpgd.c:1128). Result: every JPEG-flash attempt failed with `JPEG decode failed (jdr=5)`. The bug had been in the build the whole time but was masked for **GIF entries with a `.p4ms`**: when the JPEG flash failed, `play_current()` fell through to `load_small_gif()` and the cached canvas painted the screen anyway. **JPEG-only entries** (a fresh capture during the QUEUED / PROCESSING window before `.p4ms` is generated) hit `play_current()`'s early `return show_jpeg(ent->jpeg_path)` for `GALLERY_ENTRY_JPEG`, which fails → canvas stays at the 0x10 memset = solid blue. Fix: `JD_USE_SCALE = 1`. Other `gif_jd_decomp` callers (`gif_encoder.c::tjpgd_output`, `app_gifs.c::jpeg_crop_out_cb`) all pass `scale=0` and are unaffected. Hardware-verified — no more `JPEG decode failed (jdr=5)` log lines after the change. The author's comment in show_jpeg about "650 ms at scale 2" was the design intent that JD_USE_SCALE was supposed to enable.
- **`.p4ms` doesn't auto-play on first gallery entry during encode** — RESOLVED (2026-04-26). User report: "When I land inside the gallery [via SELECT on ALBUM from main menu] it isn't playing the .p4ms for a GIF; if I navigate to a different view and then go back it starts playing the .p4ms." Root cause: the post-format empty-overlay fix (earlier the same day) left an `app_gifs_is_encoding() || app_pimslo_is_encoding()` gate at the auto-play call site in `ui_extra_redirect_to_gifs_page`. That gate predates the `.p4ms` cached-canvas path — it was put there to prevent the GIF decoder's 7 MB allocation from colliding with the encoder's own 7 MB scaled_buf. But `app_gifs_play_current()` always tries `load_small_gif()` (the `.p4ms` path) BEFORE falling back to opening the GIF decoder, and the `.p4ms` path uses a 240×240 canvas (~115 KB × ~6 frames ≈ 700 KB) — no decoder, no collision. Skipping `play_current()` entirely was throwing out the safe `.p4ms` path along with the unsafe decoder path. Fix moves the encoder-busy gate INSIDE `play_current()`, narrowing it to JUST the `gif_decoder_open()` call. Net effect: a gallery entry with a `.p4ms` cached preview always starts playing immediately on entry, even when a different capture's `.gif` is mid-encode in the background. If the entry has only a `.gif` (no `.p4ms`) AND an encode is in flight, the GIF decoder open is skipped — the static JPEG preview already painted by `show_jpeg()` earlier in `play_current` stays on screen, same UX as the prior whole-call gate. The next gallery re-entry retries the decoder open. Also removed the redundant gate in `redirect_to_gifs_page` since `play_current` now handles the decoder collision itself.
- **Format SD vs deferred encoder window — three coupled bugs** — RESOLVED (2026-04-26). User-reported symptoms: (1) "first photo after format errors", (2) "after format → photo → gallery, P4 crashes during Queued→Processing transition; entry stays Queued forever after reboot", (3) "after format → photo → gallery, 'Album empty' overlay shown OVER the entry's PROCESSING badge + filename". Single shared root cause: `pimslo_encode_queue_task` in `app_pimslo.c` popped a job from the queue and entered a 2 s defer-poll loop while the user was on a camera page, but did NOT set `s_encoding=true` until AFTER the loop exited. So while the encoder was waiting, `pimslo_queue=0` (job already taken) AND `pimslo_encoding=0` — invisible to `format_workers_idle()` in `ui_extra.c`. User formats SD → format wipes `/sdcard/p4mslo/P4Mxxx/` → encoder eventually exits defer → encoder reads missing `pos*.jpg` → fails (or in the worst case panics on a corrupted state mid-flight). The Queued-forever symptom was the bg-encoder failure blacklist marking the post-format JPEG-only entry as broken on retry. Fix: set `s_encoding = true` + `s_encoding_num = job.capture_num` IMMEDIATELY after `xQueueReceive`, before the defer loop. Format gates on `app_pimslo_is_encoding()` so it now sees the in-flight deferred job and refuses with "busy". Defensive secondary fix: `pimslo_encode_queue_task` stat-checks for ≥2 `pos*.jpg` files before allocating the 7 MB scaled_buf — if a stale job slipped through (shouldn't happen post-fix, but cheap insurance), it logs "Skip encode P4Mxxxx — only N/4 pos files on disk" and `continue`s without the encoder ever running. Bug 3's empty-overlay issue is a separate but related fix in `ui_extra_redirect_to_gifs_page` and `app_gifs.c::scan_async_cb`: both now call `app_gifs_refresh_empty_overlay()` UNCONDITIONALLY after `app_gifs_scan()`, not just in the count==0 branch. Pre-fix the count==0 → count>0 transition (post-format → first photo encode complete) left the "Album empty" label visible on top of the new entry's PROCESSING badge. Format also now mkdirs `/sdcard/esp32_p4_pic_save` so the first P4 photo save doesn't have to take the lazy-create path. Hardware regression covered by `tests/e2e/15_format_then_photo.py`. Mock side: 6 new e2e scenarios in `test/host_encode/test_e2e.c` covering format-while-idle, format-refused-during-encode, format-refused-during-deferred-encode, post-format-first-photo, empty-overlay-zero-to-one transition, and empty-overlay-after-format.
- **`pimslo.gif` legacy filename** — RESOLVED (2026-04-26). `app_gifs_encode_pimslo_from_dir(capture_dir, ...)` derived the output GIF/.p4ms stem from the basename of `capture_dir`. The user-facing `photo_btn` flow passes `/sdcard/p4mslo/P4M%04d` → output stem `P4M0001` (correct). The legacy `spi_pimslo` / `pimslo` serial commands write 4 jpegs to `/sdcard/pimslo/posN.jpg` (literal — see `app_serial_cmd.c:586` and :615) and call `app_gifs_create_pimslo()` which encodes from `/sdcard/pimslo` → output stem `pimslo` → `pimslo.gif` in the gallery. Because `spi_pimslo` is the path used by `tests/e2e/_isolate_encode.py` and other diagnostic scripts (and the user often runs it manually before any photo_btn action), `pimslo.gif` was reliably the FIRST entry on a fresh SD card. Fix: new public `uint16_t app_pimslo_reserve_capture_num(void)` API in `app_pimslo.h` bumps the same NVS-backed counter that photo_btn uses; `app_gifs_encode_pimslo_from_dir` detects the literal-`pimslo` basename, calls the reserve API, and uses `P4M%04u` as the output stem instead. The literal serial cmd input dir stays `/sdcard/pimslo` (no need to migrate that), but every produced `.gif` and `.p4ms` is now P4Mxxxx-named so the gallery is uniform.

## Open Research Questions

(empty — `tlsf::remove_free_block`, the first-photo-after-boot use-after-free, the CUBE_IDX bit overlap, and the 0-cam silent-failure UX have all landed in a green 12/12 e2e suite as of 2026-04-26)
