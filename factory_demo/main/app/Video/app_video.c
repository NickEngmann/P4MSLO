...
/* 4 KB. A transient main-wide -O2 experiment required 8 KB (inlined
 * stack frames in frame-callback chains overflowed), but we reverted
 * to per-file -O2 on just the pure-math Gif sources — no inlining
 * changes reach this task's code path. 4 KB again. Keep buffers on
 * heap, not stack, per the Known-Issues notes. */
