...
- **Gallery shows JPEG preview for captures still encoding** — `app_gifs_scan()` merges `/sdcard/p4mslo_gifs/*.gif` with `/sdcard/p4mslo_previews/*.jpg`. A JPEG entry shows the P4 still frame via tjpgd with a center "PROCESSING" badge; once the PIMSLO encoder finishes, the next scan picks up the `.gif` and the JPEG entry for that capture is shadowed.
