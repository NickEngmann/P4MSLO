...
These regressions surfaced during the LCD/memory work but aren't root-caused yet. Each needs dedicated investigation before fixing blind:

- **Gallery `play_current` on button-driven entry takes 15-20 s** (tests 03, 10). After `cmd_btn` → `ui_extra_btn_menu` → `app_gifs_play_current`, there's a 15-20 s gap between the "Gallery entry N/M" log and the "Loaded small GIF" log. During that window `cmd_btn` holds `bsp_display_lock`, so subsequent serial commands (ping, status, sd_ls) queue up and time out. Pre-regression test 10 passed with 7 pings in this window; now we see 2-3. The delay is NOT tjpgd (preview JPEGs are 200-300 KB, ~1-2 s decode). Suspects to investigate: (1) the `.p4ms` file read from SD is somehow serialized with an unrelated SD operation, (2) `slot_find_or_alloc` is doing a large PSRAM allocation that's fragmenting-blocked, (3) `lv_obj_invalidate` after show_jpeg triggers a full 115 KB flush through the 1920-byte `trans_size` staging buffer (60 chunks) that was previously skipped when the priv-TX path was silently failing.
- **Delete modal Phase 2/3 sd_ls returns 0 entries** (test 03). After `btn_encoder` opens the delete modal and a second `btn_encoder` confirms NO, the test's followup `status` + `sd_ls` commands aren't acknowledged. Likely same root cause as the 15-20 s gallery lag above — cmd_btn is blocked inside `ui_extra_btn_encoder` + its redraw chain, so serial command dispatch stalls.
- **Test 08 rapid `photo_btn` drop** (test 08 Phase B). Python sends 3 `photo_btn` commands with 1 s gaps; only 2 reach `cmd_dispatch` (verified by counting `Command: 'photo_btn'` lines). The first one triggers `take_and_save_photo` inside the 4 KB video-stream-task callback which blocks serial I/O for the capture duration. Investigate whether lowering the capture-path work on that task (moving more into the pimslo_save task) or widening the serial RX buffer would make rapid button mashing reliable.
- **Tests 07, 08 `last_page` case mismatch** — tests compare against uppercase `'MAIN'`, but the `cmd_menu_goto` response writes lowercase (`CMD>ok page=main`). Likely a test-side bug from a period when `menu_goto` silently failed and the last page log came only from `status` responses (uppercase). Need to decide: normalize the test regex, or change `cmd_menu_goto` to echo uppercase.
- **Test 10 `page=GIFS seen` drop** — pre-regression, `status` inside each cycle reported `page=GIFS` after `btn_menu` selected ALBUM. Post-regression, `status` reports `page=CAMERA` on some cycles, suggesting the 8× btn_up / btn_down sequence is not reliably landing on ALBUM before `btn_menu` fires. Need to determine whether the home-menu selection state update is now async (e.g. deferred to an `lv_async_call`) and racing with the rapid-fire button events, or whether it was always racy and prior LVGL-wasn't-actually-flushing hid the problem.
- **S3 cameras returning 0/4 on capture** — affects tests 06, 11 and the full capture timing tests. Cameras reboot manually by the user brings them back for a window, then they drop again. Out of scope for the firmware regressions listed above but blocks the capture-path tests from running end-to-end.
