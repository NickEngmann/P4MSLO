...
- **photo_btn → tlsf::remove_free_block heap-corruption panic** (intermittent, ~50% on 2nd photo_btn after a recent encode; ~70% with TCM-LUT because encodes finish faster, leaving more "after recent encode" windows per test run) — `Guru Meditation: Store access fault` with `MEPC` inside `tlsf_control_functions.h:374`. Two reproducible call sites: (a) `app_video_photo.c:206` (`app_storage_save_picture`) → `jpeg_decoder_get_info` → `heap_caps_calloc`, (b) `app_video_stream_free_buffers` → `heap_caps_free` of `scaled_camera_buf` after a `pimslo_cap` "Freeing camera buffers" log. Both walk the TLSF free-list and dereference a garbage pointer (MTVAL ≈ 0x5474xxxx — clearly outside any valid memory region). The corruption is upstream — most likely (1) the encode pipeline's tjpgd workspace or scaled_buf overrunning, (2) the album release/reacquire JPEG-decoder dance smashing a heap header, or (3) a PSRAM-stack task overflowing into adjacent PSRAM heap blocks. Mock cannot reproduce it — `long_sequence_internal_largest_does_not_shrink` in `test/host_encode/test_e2e.c` runs 30 photo_btn cycles and the simulated allocator stays clean. So this is a concurrency/overflow issue the sequential simulator can't model, not pure fragmentation. Triage attempts: `CONFIG_HEAP_POISONING_COMPREHENSIVE=y` hangs boot at LVGL init (canary overhead too high for L2MEM headroom on this board); next try should be `CONFIG_HEAP_POISONING_LIGHT=y`. Open.
