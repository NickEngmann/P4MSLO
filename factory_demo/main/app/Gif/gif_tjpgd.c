...
)
{
	...
	rc = JDR_OK;
	unsigned int mcu_count = 0, total_mcus = (jd->width / mx) * (jd->height / my);
	for (y = 0; y < jd->height; y += my) {		/* Vertical loop of MCUs */
		for (x = 0; x < jd->width; x += mx) {	/* Horizontal loop of MCUs */
			...
			if (rc != JDR_OK) {
				/* Log which MCU failed for debugging */
				extern int printf(const char*, ...);
				printf("[tjpgd] mcu_load failed at MCU %u/%u (x=%u y=%u): %d\n",
				       mcu_count, total_mcus, x, y, rc);
				return rc;
			}
