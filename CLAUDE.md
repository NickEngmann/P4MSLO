...
- **Settings → Format SD button + serial `format_sd` — true FAT mkfs, animated, bg-coordinated** — Slot 1 of the Settings page (the old OD/Language slot — AI was stripped) is repurposed as a "Format SD" action item. Item label centered, no body text; black/clickable when SD mounted, grey/no-op when missing. SELECT opens a "Format?" YES/NO confirm modal (NO is the safe default). Confirming YES kicks off `format_bg_task` (8 KB BSS-internal stack, Core 1, prio 4 — 4 KB blew up on `app_gifs_scan()` post-format), which: (1) takes the LVGL lock briefly to call `app_gifs_stop()` + `app_gifs_flush_cache()`, (2) releases the lock, (3) calls `app_gifs_signal_bg_abort()` (new public helper, sets `s_bg_abort_current`+ bumps `s_last_nav_ms` so gif_bg pauses for 15 s), (4) polls `format_workers_idle()` for up to 5 s — `app_gifs_is_encoding() || app_pimslo_is_encoding() || app_pimslo_is_capturing() || app_pimslo_get_queue_depth()>0`, refuses with ESP_ERR_TIMEOUT if anything's still busy, (5) calls `esp_vfs_fat_sdcard_format(BSP_SD_MOUNT_POINT, card)` — true FatFs `f_mkfs` (FM_ANY, FatFs auto-picks variant: <4 GB FAT16, 4-32 GB FAT32, >32 GB exFAT if compiled in), (6) mkdirs the four PIMSLO dirs, (7) takes the lock again briefly for `app_gifs_scan()` + `app_gifs_refresh_empty_overlay()`, (8) `lv_async_call(format_done_async_cb)` to tear down the modal on the LVGL task. The modal title swaps to "Formatting" + animated dots ("Formatting" → "Formatting." → "Formatting.." → "Formatting...") via a 350 ms `lv_timer`. Animation begins immediately when SELECT YES fires (timer created BEFORE the BG task spawns) and continues throughout the bg-abort wait + the 8.5 s `f_mkfs` since the BG task only holds the LVGL lock for ms-scale pre/post bookkeeping. While `s_format_in_progress` is true the settings page ignores ALL button input (early-bail at top of UI_PAGE_SETTINGS handlers in btn_up/down/menu/encoder) so the user can't navigate away or re-confirm mid-format. Verified end-to-end on hardware 2026-04-25: SD inserted, format ran in 8.4 s, no panics, no bg-worker contention, modal closed cleanly, 5 dirs on the freshly formatted card (4 PIMSLO + 1 from app_storage init). Prior-iteration symptom: `pimslo_save_task` may be mid-fwrite when format runs and would log a write error then advance — tolerated, no crash. Caveats: format wipes EVERYTHING on the card including `/sdcard/esp32_p4_pic_save` and any user files; FAT variant is auto-picked by FatFs based on volume size (no force-FAT32 unless `CONFIG_FATFS_USE_EXFAT=n` is set in sdkconfig).
